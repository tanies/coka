...
exports.login = () => {
    ...
    return {  //api 文件夹，挂载api的对象
        ...
        ssoUrl: 'http://passport.lianjia.com',//单点登陆地址，空为自己的登陆系统
        ssoLogin: '/cas/login',   //登陆路径+参数
        ssoLogout: '/logout',  //注销路径+参数
        backQueryName: 'service',
        backToken:'token',

        sessionUrl: 'http://i.session.lianjia.com/token/verify', //有sso的session验证地址是独立的，为空时，默认为ssoUrl
        sessionMethod: 'post',
        ...
        * 其他返回值会赋值给ctx.user
        */
        checkLogin: (data) => { return { data, token: data.text } }
    }
}
